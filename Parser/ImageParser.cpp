...
#endif
        unsigned char* data_ = nullptr;//stbi_load_from_memory((const unsigned char *)buffer.buffer, buffer.size, &x, &y, &ch, 0);
        if(data_ == nullptr) {
...
#endif
        float* data_ = nullptr;//stbi_loadf_from_memory((const unsigned char *)buffer.buffer, buffer.size, &x, &y, &ch, de_ch);
        if(data_ == nullptr) {
            RK_ERROR(File, "Error: {}", stbi_failure_reason());
            return {-1, -1};
        }
